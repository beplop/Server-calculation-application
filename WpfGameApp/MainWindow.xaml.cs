...
namespace WpfGameApp
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void card_Drop(object sender, DragEventArgs e)
        {
            ...
            int networkPortsNeeded = 0;
            int totalNetworkPorts = 0;

            int kvmsNeeded = 0;
            int totalKvms = 0;

            double HDDSizeNeeded = 0;
            ...
            foreach (CardControl cell in cells) // все ячейки шкафа
            {
                if (cell.DataContext != null) // только заполненные ячейки
                {
                    if (cell.DataContext is Entities.Equipment equipment)
                    {
                        ...
                        switch(equipment)
                        {
                            case Entities.Server server:
                                row.CPU = server.CPUs;
                                row.HDD = 0.1;
                                row.Network = 4;
                                row.KVM = 1;

                                networkPortsNeeded += row.Network;
                                HDDSizeNeeded += row.HDD;
                                kvmsNeeded += row.KVM;
                                break;
                            case Entities.Storage storage:
                                row.HDD = storage.Size;
                                totalHDDSize += row.HDD;
                                break;
                            case Entities.NetworkSwitch networkSwitch:
                                row.Network = networkSwitch.Count;
                                totalNetworkPorts += networkSwitch.Count;
                                break;
                            case Entities.KvmConsole kvmConsole:
                                row.KVM = kvmConsole.Count;
                                totalKvms += row.KVM;
                                break;
                        }
            ...
            int totalPrice = list.Sum(x => x.Price);
            int totalCPU = list.Sum(x => x.CPU);

            var totalRow = new CalcRow()
            {
                Name = "Итого",
                Weight = totalWeight,
                Price = totalPrice,
            ...
            if (rack != null)
            {
                ...
                {
                    Name = "Запас",
                    Weight = rack.Capacity - totalWeight,
                    HDD = totalHDDSize - HDDSizeNeeded,
                    Network = totalNetworkPorts - networkPortsNeeded,
                    KVM = totalKvms - kvmsNeeded
                });

                totalRow.Price += rack.Price;
            }

            list.Add(new CalcRow()
            {
                Name = "$/CPU",
                Price = totalPrice / totalCPU
            });
